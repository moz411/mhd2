...
/*  htmltmpl
    ...
    CVS:            $Id: htmltmpl.php,v 1.3 2003/11/10 10:25:11 moz Exp $
*/
...
define('INT_TMPL_DATE', "\033");
define('INT_TMPL_MAIL', "\034");
define('INT_TMPL_TEXTAREA', "\035");
...
class TemplateManager {
    ...
    function TemplateManager($include=TRUE, $max_include=5, $precompile=TRUE,
                             $comments=TRUE, $gettext=TRUE, $optimize_spaces=FALSE) {
        # Constructor.
        #
    ...
    function process(&$template, $part=NULL, $direct=FALSE, 
        ...
        while (TRUE) {
            ...
            if ($token{0} == BIN_PRETOKEN) {
                ...
                if ($token == INT_TMPL_VAR) {
                    ...
                    if (! in_array($DISABLE_OUTPUT, $output_control)) {
                        ...
                        _DEB("VAR: $var");
								...
								# Added this to automagically transform unix timestamps
								# to human readable format
								elseif ($token == INT_TMPL_DATE) {
                    $var = $tokens[$i + _PARAM_NAME];
                    if (! $var) {
                        __error('No identifier in <TMPL_DATE>.');
                    }
                    $escape = $tokens[$i + _PARAM_ESCAPE];
                    $global = $tokens[$i + _PARAM_GLOBAL];
                    $skip_params = TRUE;
                    ...
                    #$value = date("d-m-Y H:i:s", $var); 
										$timestamp = $this->find_value($var, $loop_name,
                                                   $loop_pass, $loop_total,
                                                   $global);
										if(isset($timestamp) && $timestamp!="00000000000000" && $timestamp!="") {
											$value=strftime("%a %d %b %Y%n %H:%M", $timestamp);
											if(preg_match("/(.*) 00:00/",$value,$matches))
                        ...
                        _DEB("DATE: $var");
								...
								# Added this to automagically transform email variables as links
								elseif ($token == INT_TMPL_MAIL) {
                    $var = $tokens[$i + _PARAM_NAME];
                    if (! $var) {
                        __error('No identifier in <TMPL_MAIL>.');
                    }
                    $escape = $tokens[$i + _PARAM_ESCAPE];
                    $global = $tokens[$i + _PARAM_GLOBAL];
                    $skip_params = TRUE;
                    
										$mail = $this->find_value($var, $loop_name,
                                                   $loop_pass, $loop_total,
                                                   $global);
										if(isset($mail) && $mail!="")
                ...
                elseif ($token == INT_TMPL_TEXTAREA) {
                    $name = $tokens[$i + _PARAM_NAME];
                    $extra = $tokens[$i + _PARAM_INPUT_EXTRA];
                    if (! $name) {
                        __error('No identifier in <TMPL_TEXTAREA>.');
                    }
                    $skip_params = TRUE;

                    _DEB("TMPL_TEXTAREA: $name");
                    
                    if (! in_array($DISABLE_OUTPUT, $output_control)) {
                        $value = $this->find_value($name, $loop_name,
                               $loop_pass, $loop_total, FALSE);
												//$x = "<textarea name=\"".htmlspecialchars($name)."\" $extra >".htmlspecialchars(nl2br($value))."</textarea>\n";
													$x = nl2br($value);
                        if ($direct) {
                            $this->direct_output($x);
                        }
...
class TemplateCompiler {
                ...
                if ($directive == BIN_PRETOKEN.INT_TMPL_STATIC) {
                    ...
                    if (isset($this->_static[$stvar])) {
                        ...
                        __error("Cannot find STATIC data for '$st'.");
                         ...
                         $directive == BIN_PRETOKEN.INT_TMPL_TEXTAREA) {
    ...
    function find_directive(&$params) {
        ...
        switch ($directive) {
            ...
            case 'TMPL_DATE':        return INT_TMPL_DATE;
            case 'TMPL_MAIL':        return INT_TMPL_MAIL;
            ...
            case 'TMPL_TEXTAREA':       return INT_TMPL_TEXTAREA;
